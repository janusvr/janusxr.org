...
6. **Timeline** — the project's history as a walkable exhibit hall. The broader
   context is the **system map** (a subway-style transit map of virtual-world
   history, generated by `tools/history-map/` from `data/history/graph.json` —
   the node graph is the contribution surface; adapted from Shinki Nishikori's
   CC BY-SA "The timeline of virtual worlds") and the **media shelf**
   (`data/history/media.json`). Project milestones
